...
-- Full SELECT syntax (use the parts you need):
SELECT   column1, column2 AS alias, column3 * 1.14 AS calculated
FROM     table_name
WHERE    condition               -- filter rows (tomorrow's topic)
ORDER BY column1 ASC|DESC      -- sort results
LIMIT    10;                     -- cap the number of rows returned

-- Useful functions for tomorrow:
