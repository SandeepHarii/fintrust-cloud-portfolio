...
def get_cdc_latency(
    ...
    source_lag = _get_metric(
        'CDCLatencySource'
    )

    target_lag = _get_metric(
        'CDCLatencyTarget'
    )

    return {
        'cdc_latency_source_sec': source_lag,
        'cdc_latency_target_sec': target_lag,
        'cutover_safe': (
            source_lag is not None
            and source_lag < 30
        ),
...
def is_cutover_ready(
    ...
    print(
        f"Task status:    {status['status']}"
    )

    print(
        f"Source lag:     "
        f"{latency['cdc_latency_source_sec']}s"
    )

    print(
        f"Target lag:     "
        f"{latency['cdc_latency_target_sec']}s"
    )

    print(
        f"Cutover safe:   "
        f"{latency['cutover_safe']}"
    )

    return (
        status['status'] == 'running'
        and latency['cutover_safe']
    )